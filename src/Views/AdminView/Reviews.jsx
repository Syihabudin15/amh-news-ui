    ...
    const columns = [
        ...
        {title: 'Action', dataIndex: 'data', render: (data) => (
            <div style={{display: 'flex', justifyContent: 'center', flexWrap: 'wrap', gap: 10}}>
                ...
                <Button loading={load} disabled={role === 'Admin' ? false : true} size="small" onClick={() => postingPost(data._id)} 
                    icon={<CheckCircleFilled style={{color: 'green'}} />} title="Posting"></Button>
                <Button loading={load} disabled size="small" icon={<DeleteFilled style={{color: 'red'}}/>} title="Hapus" ></Button>
            </div>
        )}
    ];
    ...
    return(
        <Fragment>
            <section title="List Berita UnPosted" className="table-wrap">
                ...
                <div>
                    <Table columns={columns} dataSource={sources} loading={isLoading} pagination={false} />
                    <Pagination total={data.length} onChange={(e) => setPage(e)} style={{margin: '10px auto', textAlign: 'center'}} />
                </div>
            </section>
            <section title="List Berita UnPosted" className="table-wrap" style={{marginBottom: 100}}>
